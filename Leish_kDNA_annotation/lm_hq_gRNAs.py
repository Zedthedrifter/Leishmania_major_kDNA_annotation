...
def find_coding(k=''):
      if strand == 'coding':
        print(f'find coding {k}')
      
def extract_alignment(alignment,gRNAs, mRNAs, circle_name, minicircles, maxicircle,filter):
    mRNA_name, mRNA_align_o, gRNA_align_o, strand = alignment.split() #strand: t for template, c for coding
    ...
    #print(re.findall(anchor_seq_regex,pairing))
    ...
    #    # get the longest 5'-most anchor
        ...
        return
    #find_coding()
    #different length requirement for mini and maxi
    if 'MAXI' in circle_name.upper():
      if length < filter['min_gRNA_len_maxi'] or length > filter['min_gRNA_len_maxi']+50:
        return
    else:
      if length < filter['min_gRNA_len_mini'] or length > filter['min_gRNA_len_mini']+50:
        return
    #find_coding()
    #if length < min_gRNA_length: #filter out sequences too short
    #    return
    #length=100 #experiment, comment out later
    ...
    # find position on mRNA of gRNA and check edits occur there #okay got to keep this filter to remove false positives
    ...
    else:           #if the gRNA is from maxicircle
      if strand == 'template':
          start = str(maxicircle.seq).index(gRNA_align)
      else:
          start = str(maxicircle.seq).index(reverse_complement(gRNA_align))
    #find_coding('5')
    #filter by position: run with higher expected minicircle length such as 35 first to get the positions, then drop to 0
    if circle_name != 'Maxicircle': #this limit does not apply for maxicircles
        if start < filter['start_position'] or start > filter['end_position']:
            return
    #find_coding(6)
    ...
    gRNA['anchor_len']   = 0
    gRNAs.append(gRNA) #hmmm why not return and capture it outside?
    #return(gRNA)#hmmm not a good idea
    #find_coding(7)

def get_hq_gRNAs(mini_align_file, maxi_align_file, minicircles, maxicircle,mRNAs, filter):
    ...
    with open(mini_align_file) as f:
        for name in f:
            extract_alignment(next(f), gRNAs, mRNAs, name[1:-1], minicircles, maxicircle,filter)
            #gRNAs.append(extract_alignment(next(f), gRNAs,mRNAs, name[1:-1], minicircles, maxicircle,filter))
    with open(maxi_align_file) as f:
        for name in f:
            extract_alignment(next(f), gRNAs, mRNAs, name[1:-1], minicircles, maxicircle,filter)
    #print(gRNAs)
    # convert alignments to dataframe for filtering
    gRNAs = pd.DataFrame(gRNAs) #gRNAs is a df... handle it differently
    if len(gRNAs) == 0:
    ...
    mini_gRNA_length=min(filter['min_gRNA_len_mini'],filter['min_gRNA_len_maxi'])
    minl= gRNAs['length'] < mini_gRNA_length #filter again after trimming
    #minl = gRNAs['length'] < filter['min_gRNA_length'] #filter again after trimming
    mina = gRNAs['anchor_len'] < filter['min_anchor_length']
    maxm = gRNAs['mismatches'] > filter['max_mismatches']
    gRNAs = gRNAs[~(minl | mina | maxm)]
    #gRNAs = gRNAs[~(mina | maxm)]
    
    #different length filter for gRNA on mini and maxi

    # # collapse overlapping alignments of same mRNA and same minicircle into a longer alignment
    gRNAs = gRNAs.groupby(['mO_name', 'strand', 'mRNA_name']).apply(collapse_lm, *(minicircles, maxicircle,mRNAs), **{'post_cassette_label':False}).reset_index(drop=True)
    ...
    gRNAs = gRNAs[(gRNAs['strand'] == major_strand) | (gRNAs['mO_name']=='Maxicircle')]
    print(f'Only using gRNAs on the {major_strand} strand for minicircles\n')
...
def main(config_file='config.yaml'):
    ...
    hq_gRNAs = get_hq_gRNAs(mini_align_file, maxi_align_file,minicircles, maxicircle,mRNAs, filter)

    # get sequence around gRNA for motif searching #comment out for leish, no more motif searching
    #hq_gRNAs['sequence'] = hq_gRNAs.apply(get_sequence_lm, args=(up, down, minicircles,maxicircle), axis=1)
    ...
    axes[0].hist(hq_gRNAs[hq_gRNAs['mO_name']!='Maxicircle']['length'])
    axes[0].set_xlabel('gRNA length')
    axes[1].hist(hq_gRNAs[hq_gRNAs['mO_name']!='Maxicircle']['circle_start'], bins=range(0, hq_gRNAs[hq_gRNAs['mO_name']!='Maxicircle']['circle_start'].max()+1, 10))
