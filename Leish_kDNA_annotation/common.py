...
def get_maxicircle(filename, file_format='fasta'):
    #m = SeqIO.to_dict(SeqIO.parse(filename, file_format))
    m = SeqIO.read(filename, file_format)
    return m
...
def get_sequence_lm(gRNA, up, down, minicircles,maxicircle):
    if gRNA['strand'] == 'coding':
        s = max(0, gRNA['circle_start']-up)
        e = gRNA['circle_start']+down
    else:
...
def collapse_lm(gRNAs, minicircles,maxicircle, mRNAs, post_cassette_label=True):
    #print(gRNAs.columns)
    #print(gRNAs['anchor'])
    if len(gRNAs) == 1:
        #print('len gRNAs == 1')
        ...
        if circle_name != 'Maxicircle':
            ...
            if strand == 'coding':
                gRNA_seq = str(maxicircle.seq[min_circle_start:max_circle_end]).replace('T', 'U')[::-1]
            else:
                gRNA_seq = complement(str(maxicircle.seq[min_circle_start:max_circle_end])).replace('T', 'U')
        ...
        gRNA = {}
        #print(gRNAs.columns)#check what the keys should be
