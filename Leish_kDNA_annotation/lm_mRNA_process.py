...
def get_unprocessed_mRNAs(unedited_file, edited_file):
    ...
    for mRNA_name, mRNA in sorted(edited.items()):
        ...
        if edited_seq_T_stripped != unedited_seq_T_stripped:
            ...
            print(f'edited seq T stripped: \n{edited_seq_T_stripped}')
            print(f'unedited seq T stripped: \n{unedited_seq_T_stripped}')
            for i, (n1, n2) in enumerate(zip(edited_seq_T_stripped, unedited_seq_T_stripped)):
                if n1 != n2:
                    count=0 #find position in pre-edited sequence
                    for j,b in enumerate(unedited_seq):
                      if b!='T':
                        count+=1
                        if count==i:
                          print(f'The first mis-match ({n1, n2}) between the sequences occurs at position {j+1} of pre-edited sequence')
                    count=0 #find position in edited sequence
                    for j,b in enumerate(edited_seq):
                      if b!='T':
                        count+=1
                        if count==i:
                          print(f'The first mis-match ({n1, n2}) between the sequences occurs at position {j+1} of edited sequence')
